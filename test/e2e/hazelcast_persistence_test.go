...
var _ = Describe("Hazelcast CR with Persistence feature enabled", Label("hz_persistence"), func() {
	...
	FDescribeTable("should successfully restart from HotBackup data", Serial, func(bucketURI, secretName string) {
		if !ee {
			Skip("This test will only run in EE configuration")
		}
		...
		Entry("using GCP bucket", Label("slow"), "gs://operator-agent-backup", "br-secret-gcp"),
		Entry("using Azure bucket", Label("slow"), "azblob://backup", "br-secret-az"),
	)
})
